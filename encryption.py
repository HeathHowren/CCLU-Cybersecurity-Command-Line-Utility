...
def AESmain():
    ...
    # convert bit keysize to bytes
    keySizeByte = keySize // 8
    # test if padding is needed based on keysize
    if len(user_input) % keySizeByte != 0:
        # pad message
        user_input = pad(user_input, 16)
    print("AES-" + str(keySize) + "-" + mode)
    cipher = ""
    if mode == "ECB":
        # XOR each block using the key
        for i in range(0, len(user_input), keySizeByte):
            # get current block
            currentBlock = user_input[i:i + keySizeByte]
            # XOR current block with key
            currentBlock = bytes(a ^ b for a, b in zip(currentBlock, keySizeByte))
            # apply the AES encryption algorithm to the current block
                #  substitution-permutation network
                #  each round, the 128-bit block of plaintext is substituted with a new 128-bit block of ciphertext using a substitution table
                #  the substitution table is generated by the key
                #  the key is also used to permute the order of the blocks of ciphertext
            
            # add current block to cipher
            cipher += currentBlock.decode()
        print("Encrypted message: " + cipher)
