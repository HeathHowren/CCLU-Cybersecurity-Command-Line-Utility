...
def pad(message_bytes, block_size):
    # pad message_bytes
    # get length of message_bytes
    length = len(message_bytes)
    # get number of blocks
    num_blocks = length // block_size
    # get number of padding characters needed
    num_padding_chars = block_size - (length % block_size)
    # add padding characters
    for i in range(num_padding_chars):
        message_bytes += b'\x00'
    return message_bytes

# AES dependencies * v o m i t s *
def generate_substitution_table(key):
    # Generate the substitution table using the key
    # This is just a placeholder function, the actual implementation would depend on the specific requirements of the application
    substitution_table = key
    return substitution_table

def substitute_bits(block, substitution_table):
    # Substitute the bits in the block using the substitution table
    # This is just a placeholder function, the actual implementation would depend on the specific requirements of the application
    substituted_block = block
    return substituted_block

def permute_bits(block, key):
    # Permute the bits in the block using the key
    # This is just a placeholder function, the actual implementation would depend on the specific requirements of the application
    permuted_block = block
    return permuted_block

def apply_aes_ecb(block, key):
    # Generate the substitution table using the key
    substitution_table = generate_substitution_table(key)

    # Substitute the bits in the block using the substitution table
    substituted_block = substitute_bits(block, substitution_table)

    # Permute the bits in the block using the key
    permuted_block = permute_bits(substituted_block, key)
...
def AESmain():
    ...
    user_input = input("Please enter a message: ")
    print()
    # ask for a cipher mode of encryption (ECB or CBC)
    print("Cipher modes of encryption:")
    print("1. ECB")
    print("2. CBC")
    mode = ""
    option_input = input("Please enter a number: ")
    if option_input == "1" or option_input == "ECB":
        mode = "ECB"
    elif option_input == "2" or option_input == "CBC":
        mode = "CBC"
    else:
        print("Invalid input. Please try again.")
        option_input = input("Please enter a number: ")
    # ask for and store key size
    keySize = 0
    print("Key sizes in Bits:")
    print("1. 128")
    print("2. 192")
    print("3. 256")
    option_input = input("Please enter a number: ")
    if option_input == "1" or option_input == "128":
        keySize = 128
    elif option_input == "2" or option_input == "192":
        keySize = 192
    elif option_input == "3" or option_input == "256":
        keySize = 256
    else:
        print("Invalid input. Please try again.")
        option_input = input("Please enter a number: ")
    # transform message into bytes
    user_input = user_input.encode()
    # convert bit key size to bytes
    keySizeByte = keySize // 8
    # create a random key
    key = os.urandom(keySizeByte)
    # test if padding is needed based on key size
    if len(user_input) % keySizeByte != 0:
        # pad message
        user_input = pad(user_input, keySizeByte)
    print("AES-" + str(keySize) + "-" + mode)
    print("Key (bytes): " + str(key))
